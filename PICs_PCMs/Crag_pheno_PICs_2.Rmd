...
*************************************
Match data to phylogeny and phylosig
*************************************

```{r}
#read in initial phylogeny from MrBayes and parse data 
cragphy<-read.tree("Cannizzaro_COI_and_Crangon_34_phy.nex")  
...
this = read.csv("matched_phenotypes.csv", row.names=1, stringsAsFactors = TRUE)

hab = setNames(this[,1],rownames(this))

plotTree(tree, fsize=1, ftype="i", lwd=1, offset=1)

#hab_cols = setNames(c("darkseagreen", "indianred1", "gray35"),levels(hab))
hab_cols = setNames(c("darkseagreen", "rosybrown4", "paleturquoise1"),levels(hab))

tiplabels(pie=to.matrix(hab[tree$tip.label],
    levels(hab)),piecol=hab_cols,cex=1)

fitARD = ace(hab, tree, model="ARD", type="discrete")

nodelabels(node=1:tree$Nnode+Ntip(tree),
    pie=fitARD$lik.anc,piecol=hab_cols,cex=1)

add.simmap.legend(colors=hab_cols,prompt=FALSE,x=0.5,
    y=5.5,fsize=0.5)
...
##### With contor colors #####
##### Use hab_col from above for pies #####
...
#add.color.bar(0.15, obj$cols,title="behavior",lims=obj$lims,digits=2,prompt=FALSE,x=0.525,y=4,lwd=3,fsize=0.5,subtitle="", outline=FALSE)
...
#add.color.bar(0.15, obj$cols,title="respiratory",lims=obj$lims,digits=2,prompt=FALSE,x=0.525,y=4,lwd=3,fsize=0.5,subtitle="", outline=FALSE)
...
mod1 = lm(dat[,3]~dat[,6]) #simople linear model of CVs
print("*******************************Mod1************************************")
summary(mod1)
print("*******************************************************************")
...
habA.tr<-read.tree("species_habitat.tre")
habB.tr<-read.tree("species_habitat2.tre")


#below code is for checking and modifying trees if needed
is.binary(habA.tr)
is.ultrametric(habA.tr)
is.binary(habB.tr)
is.ultrametric(habB.tr)

ultraHabA = force.ultrametric(habA.tr, method=c("nnls","extend"))
ultraHabB = force.ultrametric(habB.tr, method=c("nnls","extend"))

is.binary(ultraHabA)
is.ultrametric(ultraHabA)
is.binary(ultraHabB)
is.ultrametric(ultraHabB)
...
write.tree(ultraHabA, file = "ultraHabA.tre")
write.tree(ultraHabB, file = "ultraHabB.tre")

ultraHabA2<-read.tree("ultraHabA2.tre")
ultraHabB2<-read.tree("ultraHabB2.tre")

is.binary(ultraHabA2)
is.ultrametric(ultraHabA2)
is.binary(ultraHabB2)
is.ultrametric(ultraHabB2)


plot(ultraHabA2) #use these trees for analysis
plot(ultraHabB2) #use these trees for analysis
...
```{r}
# behavior means
tr1 = read.tree("species_habitatA.tre")
...
beh.BM<-fitContinuous(tr, behCV, model="BM")
beh.WN<-fitContinuous(tr, behCV, model="white")
beh.BM.hab<-fitContinuous(tr1, behCV, model="BM") 
...
```{r}
# respiritory CVs
resp.BM<-fitContinuous(tr, respCV, model="BM")
resp.WN<-fitContinuous(tr, respCV, model="white")
resp.BM.hab<-fitContinuous(tr1, respCV, model="BM") 
...
######### Topology 2 ######### This is idnetical from above, just different projection
